...
/*   Updated: 2023/06/07 09:57:54 by snaggara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_display_map(t_canva *canva)
{
	...
	while (browse)
	{
		ft_printf("Addrs : %p, x : %d, y : %d, type : %c, gauche : %p, droite %p, haut %p, bas %p\n", browse, browse->x, browse->y, browse->type, browse->lf, browse->rg, browse->up, browse->dw);
		browse = browse->next;
	}
	browse = canva->map_first;
	i = 0;
	...
	{
		ft_printf("Addrs : %p, x : %d, y : %d, type : %c\n", canva->map_items[i], canva->map_items[i]->x, canva->map_items[i]->y, canva->map_items[i]->type);
		i++;
	}
